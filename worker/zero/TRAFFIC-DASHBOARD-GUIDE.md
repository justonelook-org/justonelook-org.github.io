...
- **Roads to Zero:** arrivals through implemented clean source paths. The currently published routes are `/try-it/x/`, `/try-it/bluesky/`, `/try-it/reddit/`, and `/try-it/tiktok/`. Each records one approved source count and immediately continues to canonical `/try-it/`.
...
Each source path sends only its fixed approved source slug: `x`, `bluesky`, `reddit`, or `tiktok`. It does not send or store a referrer. Source totals are not joined to page openings, sessions, conversations, or outcomes. `/try-it/` remains the canonical Looking Zero URL, and the source-entry pages are excluded from search indexing.
...
Source counts answer only how often each approved road was opened. They do not show whether those arrivals became Zero openings, sessions, invitations, or indications of trying.
